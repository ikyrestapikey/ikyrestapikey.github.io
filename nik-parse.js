...
export default async function handler(req, res) {
    const endpoint = '/api/nik-parse';
    const { nik } = req.query;

    if (!nik) {
        await logApiRequest(endpoint, 400);
        return res.status(400).json({ success: false, error: 'Query parameter "nik" is required.' });
    }

    try {
        const result = await nikParse(nik);
