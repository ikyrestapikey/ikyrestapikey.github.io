...
class VertexAI {
    ...
    chat = async function (question, { model = 'gemini-1.5-flash', system_instruction = null, search = false } = {}) {
        ...
        if (r.status !== 200 || !r.data.candidates) throw new Error('No result found from the API.');
    ...
    image = async function (prompt, { model = 'imagen-3.0-generate-002', aspect_ratio = '1:1' } = {}) {
        ...
        const r = await axios.post(`${this.api_url}/${this.model_url}/${model}:predict`, {
            ...
            parameters: {
                sampleCount: 1, includeRaiReason: true, aspectRatio: aspect_ratio,
                safetySetting: 'block_only_high', personGeneration: 'allow_adult',
                addWatermark: false, imageOutputOptions: { mimeType: 'image/jpeg', compressionQuality: 100 }
            }
        }, {
            headers: this.headers
        });
        
        if (r.status !== 200 || !r.data.predictions) throw new Error('No result found from the API.');
